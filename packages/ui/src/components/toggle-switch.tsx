...
>(({ onOff = "true,false", className, checked, ...props }, ref) => {
  let on: string
  let off: string
  ...
  return (
    ...
    >
      <SwitchPrimitives.Thumb
        className={cn(
          "absolute pointer-events-none block h-full w-[50%] bg-background rounded-md shadow-lg ring-0 transition-transform data-[state=checked]:translate-x-0 data-[state=unchecked]:translate-x-[100%]"
        )}
      />
      <div className='relative grid gap-2 grid-cols-2 p-2 justify-center items-center w-full'>
        <div className='flex justify-center px-1 text-foreground'>{on}</div>
        <div
          className={cn(
